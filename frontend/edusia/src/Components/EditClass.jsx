...
const EditClass = ({data, currentUser}) => {
    const [teacher, setTeacher] = useState({
        name: data.classes.name,
        id: data.classes.teacher_id,
        username: data.classes.username,
        picture: data.classes.picture
    });
    const [subject, setSubject] = useState(data.classes.subject);
    const [class_code, setClass_code] = useState(data.classes.class_code);
    ...
    return (
        <>
            <form method="PUT" onSubmit={editClass}>
                ...
                :
                        ...
                        {students && students.map((student, i) => {
                            return (
                                <div className="cardPair" key={i}>
                                    ...
                                    <button className="buttonOrange" type="button" onClick={() => {removeStudent(student.id)}}>Remove</button>
                                </div>
                            )
                        })}
                    </>
                 ...
                 <div style={{margin: "40px 0 0 0"}} className="formSubmit">
                    <input className="buttonBlue" type="submit" value={`Update Class`} />
                </div>
            </form>
