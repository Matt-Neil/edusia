...
exports.getLesson = async (req, res, next) => {
    try {
        const lesson = await db.query("SELECT classes.subject, classes.class_code, users.name, users.picture FROM classes INNER JOIN users ON classes.id = $1 AND (classes.school_id = $2 OR classes.teacher_id = $2) AND users.id = classes.teacher_id", 
...
exports.addLesson = async (req, res, next) => {
    try {
        if (res.locals.currentUser.position === "school") {
            await db.query("INSERT INTO classes (teacher_id, school_id, subject, class_code) VALUES ($1, $2, $3, $4) returning *",
                [req.body.teacher_id, req.body.school_id, req.body.subject, req.body.class_code]);

            //INSERT NEW STUDENTS INTO STUDENTS_CLASSES
